					...
					$("#viewTestsButton").click(function(){
						$.post("PHPScripts/admin/getReports.php",{
							option: "getCreatedTests"
								...
								var htmlToAdd = "";
								$.each(data, function(key,value){
									$("#infoTable").append("<tr><td><button id='t"+value.genTestID+"'>Questions</button></td><td>"+value.genDate+"</td><td>"+value.course_type+"</td><td><i>"+value.testPassword+"</i></td><td>"+value.avg+"</td></tr><tr><td>Per Category Averages for Class:</td></tr>");
									$.each(value.perSubcatAnalysis, function(key,value){
										$("#infoTable").append("<tr><td>"+key.toUpperCase()+" Average Score: "+value+"%</td></tr>");
									});
								});
