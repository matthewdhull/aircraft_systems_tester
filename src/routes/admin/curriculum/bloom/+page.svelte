<script lang="ts">
	...
	import BloomVocabulary from '$lib/components/curriculum/BloomVocabulary.svelte';
	import ErrorSummary from '$lib/components/ErrorSummary.svelte';
	import { tick } from 'svelte';
	import type { ActionData, PageData } from './$types';

	let { data, form }: { data: PageData; form: ActionData } = $props();
	const fieldSuffix: Record<string, string> = { name: 'name', ordinal: 'ordinal', verb: 'verb' };
	const summaryErrors = $derived([
		...
		...(form?.error
			? [{ message: `The vocabulary change failed: ${form.error.replaceAll('_', ' ')}.` }]
			: [])
	]);
...
<svelte:head><title>Bloom vocabulary | Curriculum modeling</title></svelte:head>

<section aria-labelledby="bloom-heading">
	<p class="eyebrow">Curriculum modeling</p>
	<h1 id="bloom-heading" tabindex="-1">Bloom vocabulary</h1>
	<p>Manage controlled levels and verbs without inventing or seeding production taxonomy.</p>
	<nav class="section-navigation" aria-label="Curriculum management">
		<a href={resolve('/admin/curriculum')}>Hierarchy</a>
		<a aria-current="page" href={resolve('/admin/curriculum/bloom')}>Bloom vocabulary</a>
		<a href={resolve('/admin/curriculum/mappings')}>Legacy mapping review</a>
	</nav>
	{#if form?.success}<p class="notice" role="status">Bloom vocabulary updated.</p>{/if}
	{#if navigating.to}<p class="loading" role="status">Loading Bloom vocabulary…</p>{/if}
	{#if form && !form.success}<ErrorSummary errors={summaryErrors} />{/if}
	<BloomVocabulary levels={data.levels} dependencyWarnings={data.dependencyWarnings} />
</section>
